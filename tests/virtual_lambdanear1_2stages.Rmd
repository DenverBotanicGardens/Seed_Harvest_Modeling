...
# Annuals
```{r}
annuals <- lapply(1:100, function(x) MPM_annual(paramsAll, maxSurv = 0.8, minSurv = 0.001))
...
mean(lamannuals$lam)
sd(lamannuals$lam)
generation.time(mean(MPMs_annuals))
mean(unlist(lapply(annuals, function(x) x[[3]]$gentim)))

params_annuals <- do.call(rbind,lapply(annuals, function(x) x[[4]]))
params_annuals <- params_annuals[!duplicated(params_annuals[,1:2]),]
