...
# Age_1 <- 2
# Age_2 <- 4
rm(alpha2);rm(beta2);rm(Age_1);rm(Age_2);rm(survival1);rm(survival2)
...
#   rnorm(1, survival1 - survival2, 0.05)
# }

Age_first_1 <- 0.5
Age_first_2 <- 3
alpha2 <- params_itero[1,1]
beta2 <- params_itero[1,2]
...
Growth <- function(Age_first_1, Age_first_2, alpha2, beta2){
  ...
  matching_proportion <- survivalTypeI(alpha2, beta2, Age_first_2-1)/
    sum(survivalTypeI(alpha2,beta2,Age_first_1:Age_first_2))
  matching_proportion
}

# Growth <- function(Age_1, Age_2){
#   parameters <- c(veg_age = Age_1, rep_age = Age_2)
#   state <-c()
#   
#   # x is age
#   delta_veg <- -m(x)*n_veg(x) - h(x)*n_veg(x)
#   delta_rep <- m(x)*n_veg(x) - a*n_rep(x)
# }

# Growth <- function(alpha2, beta2, Age_1, Age_2){
#   (survival1 <- exp((alpha2/beta2)*(1-(exp(beta2*Age_1)))))
#   (survival2 <- exp((alpha2/beta2)*(1-(exp(beta2*Age_2)))))
#   rnorm(1, (survival1 + survival2)/2, 0.05)
# }
...
MPM_semel <- function(params, stage1, stage2){
  ...
  (f <- itero_fecundsurv(s_s[2]))
  # transitions or growth based on how far away (how big a reduction along survival curve) to get from stage 1 to 2
  (G <- Growth(alpha2 = params[i,1], beta2 = params[i,2],Age_1 = stage1, Age_2 = stage2+(stage2 - stage1)))
  # Of the ones that survive, s_s[1], what proportion grow? 
  t_ij <- matrix(c(s_s[1]-G, G,
                   f, 0),
                 nrow = 2)
...
params8 <- params[params$a2b2 %in% 
                    params$a2b2[params$survival < 0.9 & params$age == stage1],]  
params8 <- params8[params8$a2b2 %in%
                    params8$a2b2[params8$survival > 0.76 & params8$survival < 0.8 & params8$age == stage2],]
params8 <- params8[!duplicated(params8[,c("a2","b2")]),]
        a2   b2 age  survival     a2b2
18201 0.05 0.10 0.5 0.9746903  0.050.1
18241 0.05 0.11 0.5 0.9746272 0.050.11
18281 0.05 0.12 0.5 0.9745639 0.050.12
18321 0.05 0.13 0.5 0.9745004 0.050.13
18361 0.05 0.14 0.5 0.9744366 0.050.14
18401 0.05 0.15 0.5 0.9743727 0.050.15

Don't know what parameters were for the following but it was decent too

        a2   b2 age  survival     
8961  0.02 0.52 0.5 0.9886446
9001  0.02 0.53 0.5 0.9886151
12001 0.03 0.37 0.5 0.9836578
15161 0.04 0.25 0.5 0.9789216
18401 0.05 0.15 0.5 0.9743727

# With growth over survival curve but all lambdas over 1
# params8 <- params[params$a2b2 %in% 
#                     params$a2b2[params$survival < 0.99 & params$age == stage1],]  
# params8 <- params8[params8$a2b2 %in%
#                     params8$a2b2[params8$survival > 0.7 & params8$survival < 0.8 & params8$age == stage2],]
# params8 <- params8[params8$a2b2 %in%
#                     params8$a2b2[params8$survival > 0.43 & params8$survival < .51 & params8$age == stage2+2],]

# before changes to growth
# params8 <- params8[params8$a2b2 %in%
#                     params8$a2b2[params8$survival > 0.7 & params8$survival < 0.747 & params8$age == stage2],]
# params8 <- params8[params8$a2b2 %in%
#                     params8$a2b2[params8$survival > 0.71 & params8$survival < 0.8 & params8$age == stage2],] #
...
othermoving <-  -0.08
params8 <- params8[params8$a2b2 %in%
                    params8$a2b2[params8$survival > (0.73 + othermoving) & 
                                   params8$survival < (0.8 ) & params8$age == stage2],]
moving <- -0.04
params8 <- params8[params8$a2b2 %in%
                    params8$a2b2[params8$survival > (0.47 - moving) & 
                                   params8$survival < (.51 - moving) & params8$age == stage2+2],]
...
plot(0:11, survivalTypeI(params8[1,1], params8[1,2], 0:11), type = "l", col = rainbow(nrow(params8), alpha = 1)[1],
     # xlab = "",xaxt = "n", 
     xlab = "Age in years",
     ylab = "Survival (stasis)",
     ylim = c(0,1))
for(i in 2:nrow(params8)){
  lines(0:11, survivalTypeI(params8[i,1], params8[i,2], 0:11), type = "l", col = rainbow(nrow(params8), alpha = 1)[i])
}
...
#                         params$a2b2[params$survival < 0.95 & params$age == stage1],]
# params8slow <- params8slow[params8slow$a2b2 %in%
#                     params8slow$a2b2[params8slow$survival < 0.8 & params8slow$age == stage2],]
# params8slow <- params8slow[params8slow$a2b2 %in%
#                     params8slow$a2b2[params8slow$survival < 0.8 & params8slow$survival > 0.6 & params8slow$age == stage2+2],]
# params8slow <- params8slow[!duplicated(params8slow[,c("a2","b2")]),]
# 
# params_iteroslow <- params8slow

# save(params_iteroslow, file = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/paramsIteroSlow.Rdata")
