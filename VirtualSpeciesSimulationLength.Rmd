...
MatrixType <- "Annual"
Simlength <- 100
# rm(Mx_list);rm(vec);rm(Mx_sample);rm(TMx_list)
...
virtualPVA <- function(Mx_list, Nxstages = rep(1,4), StartPopSize, MatrixType, Simlength, generationspan = FALSE){
  ...
  Time2Ext <- NA
  # PVA loop
  dfout <- foreach(i = 1:projlength, .combine = 'c') %dopar% {
    yr[i] <- i
    ...
    if(i == 1){
      ...
      Tulapprox[i] <- popbio::stoch.growth.rate(mats, verbose = FALSE)$approx
      LogGrowthsim[i] <-  popbio::stoch.growth.rate(mats, verbose = FALSE)$sim
      LogGrowthSimLowerCI[i] <-  popbio::stoch.growth.rate(mats, verbose = FALSE)$sim.CI[1]
      LogGrowthSimUpperCI[i] <-  popbio::stoch.growth.rate(mats, verbose = FALSE)$sim.CI[2]
      }
