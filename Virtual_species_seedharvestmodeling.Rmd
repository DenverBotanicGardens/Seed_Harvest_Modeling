...
# limited reasoning to use dirichlet this way
# compadre annual is 4.857 generation time
# Tmx_annual <- lapply(1:100, function(rep){
#   # three seed stages, 1 reproductive
#   # lower adult survival then reproduction is higher
#   f <- runif(1,0,10)
#   a.mat <- matrix(c(1,f*3,1,f*3,1,1,f*3,f*3,1), nrow = 3, byrow = TRUE)
#   # a.mat <- matrix(c(10,10,1,20,1,1,20,20,1), nrow = 3, byrow = TRUE) # transpose row and columns now
#   t_ij <- t( as.matrix(round(rdirichlet(3, a.mat),2)) )
#   t_ij[1,2:3] <- 0
#   t_ij[2,c(1,3)] <- 0
#   t_ij <- cbind(t_ij,rep(0,3))
#   matrix(c(0,0,0,f,t(t_ij)), nrow = 4, byrow = TRUE)
# })
...
Tmx_annual <- lapply(1:100, function(rep){
  ...
  t_ij[4,1:3] <- runif(3, min = 0, max = s)
  t_ij[1,4] <- f
...
Tmx_semelslow <- lapply(1:100, function(Mx){
  f <- runif(1,0,10)
  t_ij <- as.matrix(rdirichlet(3, runif(3, min = 1, max = 10)))
  if(any(colSums(t_ij)>=1)){
    for(i in which(colSums(t_ij)>=1)){
      t_ij[,i] <- t_ij[,i]/ (sum(t_ij[,i]) + runif(1,0.01,0.2)) # make there be some death, 80-99% survival
    } 
  } 
  
})

Tmx_itero <- lapply(1:100, function(Mx){
  s <- runif(1,0,0.8) # adult survival
  f <- -12.5 * ((s + 0.1)^2) + 10.125 + rnorm(1, mean = 0, sd = 0.1) # add some variance around the line
  t_ij <- as.matrix(rdirichlet(3, runif(3, min = 1, max = 20)))
  if(any(colSums(t_ij)>=1)){
    for(i in which(colSums(t_ij)>=1)){
      t_ij[,i] <- t_ij[,i]*(sum(t_ij[,i]))
    } 
  } 
  t_ij.semel <-  as.matrix(rdirichlet(3, runif(4, min = 1, max = 20)))
  TMx_itero <- matrix(c(0,0,0,f,t(t_ij)), nrow = 4, byrow = TRUE) 
  TMx_semel <-matrix(c(0,0,0,f,t(matrix(c(t_ij[,-4],rep(0,3)), nrow = 3))), nrow = 4, byrow = TRUE) 
