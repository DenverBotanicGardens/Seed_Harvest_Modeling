...
# Lambda
# ggplot(toplot[toplot$StPopSz == 500,], aes(SimLengthtext, exp(Tulapprox)))+
...
#   ggtitle("a)")+
#   facet_wrap(~MatrixType) #, labeller = labeller(MatrixType = c("Annual","Semelparous Fast","Iteroparous Fast","Semelparous Slow","Iteroparous Slow")))
...
# time to extinction
time2extplot <- ggplot(toplot, aes(SimLengthtext, Time2Ext, colour = as.factor(StPopSz)))+
  # geom_jitter(height = 0, width = 2, alpha = 0.4, shape = 20)+
  # geom_boxplot(x = as.factor(toplot$generationtime[toplot$StPopSz == 500]))+
  geom_boxplot()+
  theme_bw()+
  ...
  xlab("Simulation length")+
  ggtitle("b) ")+
  facet_wrap(~MatrixType, ncol = 2)+
  scale_colour_discrete(name = "Initial\npopulation\nsize")



ggplot(toplot[toplot$StPopSz == 500,], aes(as.factor(generationtime), exp(Tulapprox) , colour = as.factor(SimLength)))+
  # geom_jitter(height = 0, width = 2, alpha = 0.4, shape = 20)+
  # geom_boxplot(x = as.factor(toplot$generationtime[toplot$StPopSz == 500]))+
  geom_boxplot()+
  theme_bw()+
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Virtualextinctionmetrics.jpg",
       ERplot + time2extplot +plot_layout(widths = c(2,3)),
       width=230, height=100,units='mm', dpi=300)
```
...
pathstart100 # "No10years
pathstartVirtual100 # "No100years"
...
pathstart100 # "AnnualEmpericalNone"
...
toplotEmp100 <- pullresults("No100years", pathstartVirtual100, "100 years")
toplotEmp100annuals <- pullresults("AnnualEmpericalNone", pathstart100, "100 years")
toplotEmp <- do.call(rbind, list(toplotEmp3,toplotEmp10,toplotEmp100))

# Label the empirical species by annual, fast and slow itero or semelparous
sp_infoCIPIone <- sp_info[-nrow(sp_info),]
sp_infoCIPIone$SPP <- as.character(sp_infoCIPIone$SPP)
sp_infoCIPIone$SPP[sp_infoCIPIone$SPP == "CIPI3"] <- "CIPI"
# Add two annuals
...
simgenlengthEmp1 <- ggplot(toplotEmp1[toplotEmp1$StPopSz == 500 & toplotEmp1$SimLengthtext == "3 generations",], 
       aes(Iteroparity, generationtime*3))+ # , colour = MatrixType))+
  geom_boxplot()+
  theme_bw()+
  ylab("Simulation length\n years")+
  xlab("Species type")+
