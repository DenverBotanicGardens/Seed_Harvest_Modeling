...
allsims <- do.call(rbind, lapply(unique(tm$SPP)[-2], function(sp){ # [-2]CIPI has one population with different number of stages
  # for(GB in 1:4){
    startszout <- do.call(rbind, lapply(1:1, function(popsz){
      clusterout <- do.call(rbind, lapply(1:3, function(csz){ # clustered collection number of years
          # GoodBad <- c("random","mostlybad","mildclustered","mostlygood")
          # GBmatrix <- list(matrix(c(0.5,0.5,0.5,0.5), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad"))), # random
                           # matrix(c(0.25,0.75,0.25,0.75), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad"))), # extreme drought
                           # matrix(c(0.55,0.45,0.45,0.55), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad"))), # mild clustered
                           # matrix(c(0.75,0.25,0.75,0.25), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad")))) # mostly good
          ...
          save(noANDharvest, file = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/noANDharvest",sp,"popsz",startpop[popsz],"clustersz",clucol[csz],"reps",rep_gb,".Rdata",sep=""))
          harvestlevels <- levels(factor(noANDharvest$Harvest))
          if(any(!is.na(noANDharvest$Yr2Ext[noANDharvest$Harvest == "0:0:0:0"])) & 
             any(!is.na(noANDharvest$Yr2Ext[noANDharvest$Harvest != "0:0:0:0"]))){ # Does the population evenutally go to extinction?
            diff.time2ext <- do.call(rbind,lapply(harvestlevels[harvestlevels!="0:0:0:0"] ,function(harv){ # how many columns of frequency and intensity rates
              diff.freq <- do.call(rbind,lapply(1:1000, function(x){
                # eventually check across levels of harvest, now is levels of good to bad
                noharvest <- noANDharvest[noANDharvest$Harvest=="0:0:0:0" & !is.na(noANDharvest$Yr2Ext),]
                harvest <- noANDharvest[noANDharvest$Harvest==harv & !is.na(noANDharvest$Yr2Ext),]
                har_idx <- sample(1:(nrow(harvest)), size = 1)
                noharv <-sample(noharvest$Yr2Ext, size = 1)
                out <- data.frame(DiffTime2Ext = (noharv-(harvest$Yr2Ext[har_idx]))/noharv, 
                                  GB_matrix = harvest$GB_mat[har_idx], DistGB = harvest$DiffGB_mat[har_idx],
                                  Harvest = harv, lifespan = floor(lifespan.sp$lifespan[lifespan.sp$SPP==sp]))
                out
                }))
              diff.freq
          ...
          if(any(is.na(noANDharvest$Yr2Ext[noANDharvest$Harvest == "0:0:0:0"])) & 
             any(is.na(noANDharvest$Yr2Ext[noANDharvest$Harvest != "0:0:0:0"]))){ # Some populations do not go extinct
            diff.endpopsz <- do.call(rbind,lapply(harvestlevels[harvestlevels!="0:0:0:0"] ,function(harv){ # how many columns of frequency and intensity rates
              diff.freq <- do.call(rbind,lapply(1:1000, function(x){
                # eventually check across levels of harvest, now is levels of good to bad
                ...
                har_idx <- sample(1:nrow(harvest), size = 1)
                noharv <- sample(noharvest$EndPopSz, size = 1)
                out <- data.frame(DiffEndPopSz = ((log(noharv)-(log(harvest$EndPopSz[har_idx]))))/(log(noharv)), 
                                  GB_matrix = harvest$GB_mat[har_idx], DistGB = harvest$DiffGB_mat[har_idx],
                                  Harvest = harv, lifespan = floor(lifespan.sp$lifespan[lifespan.sp$SPP==sp]))
                out
                }))
...
for(harvlev in harvestlevels[harvestlevels!="0:0:0:0"]){
  #GoodBad[GB], take out when it's random, not discrete
  if(exists("diff.time2ext")){
    ggsave(filename = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/","StartPopSz",startpop[popsz],"yrColl",clucol[csz],"_",sp,"_diffEndPopSzXGB.jpg",sep=""),
           ggplot(diff.time2ext[diff.time2ext$Harvest == harvlev,], aes(DistGB, DiffTime2Ext))+
             geom_point()+
             theme(legend.position="none",
                   axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
                   panel.background = element_rect(fill = "white", colour = "grey75"))+
             geom_smooth(method = "lm")+
             labs(title = paste(sp,"Time to extinction at harvest level:",harvlev),
                  x = "Periodicity",
                  y = "Percent of no harvest"),
           width=300, height=300,units='mm', dpi=300)
  } # Only run if comparison exists
  if(exists("diff.endpopsz")){
    # difference from 100% being no harvest; #,GoodBad[GB]
    ggsave(filename = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/","StartPopSz",startpop[popsz],"clustersz",clucol[csz],"_",sp,"_difftime2extXGB.jpg",sep=""),
           ggplot(diff.endpopsz[diff.endpopsz$Harvest==harvlev,], aes(DistGB, DiffEndPopSz))+
             geom_point()+
             theme(legend.position="none",
                   axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
                   panel.background = element_rect(fill = "white", colour = "grey75"))+
             geom_smooth(method = "lm")+
             labs(title = paste(sp,"Change in (log) population size at harvest level:",harvlev),
                  x = "Periodicity",
                  y = "Percent of no harvest"),
           width=300, height=300,units='mm', dpi=300)
    } # Only run if exists
...
startszout
# } # when for loop of discrete GB good bad matrices
    ...
    dfs <- do.call(rbind,lapply(resultpath[-1], function(x){
      ...
      diff.time2ext <- do.call(rbind,lapply(harvestlevels[harvestlevels!="0:0:0:0"] ,function(harv){ # how many columns of frequency and intensity rates
          ...
          diff.GBmats <- do.call(rbind, lapply(unique(noANDharvest$GB_mat), function(GB){
            if(any(!is.na(noANDharvest$Yr2Ext[noANDharvest$Harvest=="0:0:0:0" & noANDharvest$GB_mat == GB]))){ # Does the population evenutally go to extinction at this GB matrix?
...
table(harvestsims[[1]][[2]]$StartPopSz,harvestsims[[1]][[1]]$CollClu)

library(stringr)

# get error for very large numbers with EndPopSz
i <- 1 # for the starting population sizes (now should be 4 but this has three, missing starting with 10)
j <- 3 # each sub-list has three, 1, 3, and 5 consecutive years of collections
...
harvestsims_endpop <- lapply(startpop, function(s){
  clusters <- lapply(clucol, function(cl){
    ...
    dfs <- do.call(rbind,lapply(resultpath[-1], function(x){
      load(x)
      harvestlevels <- levels(factor(noANDharvest$Harvest))
      if(is.na(any(noANDharvest$Yr2Ext))){ # Some populations do not go extinct
          diff.EndPop <- do.call(rbind,lapply(1:1000, function(x){
            # eventually check across levels of harvest, now is levels of good to bad
            noharvest <- noANDharvest[noANDharvest$Harvest=="0:0:0:0" & !is.na(noANDharvest$EndPopSz),]
            harvest <- noANDharvest[noANDharvest$Harvest==harv & !is.na(noANDharvest$EndPopSz),]
            har_idx <- sample(1:nrow(harvest), size = 1)
            out <- data.frame(DiffEndPopSz = log(harvest$EndPopSz[har_idx])/log(sample(noharvest$EndPopSz, size = 1)), # (log(sample(noharvest$EndPopSz, size = 1))-(log(harvest$EndPopSz[har_idx]))), 
                              GB_matrix = harvest$GB_mat[har_idx], DistGB = harvest$DiffGB_mat[har_idx],
                              Harvest = harv, lifespan = unique(noANDharvest$lifespan), SPP = unique(noANDharvest$SPP),
                              StartPopSz = s, CollClu = cl)
            out
...
table(harvestsims_endpop[[1]][[2]]$StartPopSz,harvestsims_endpop[[1]][[1]]$CollClu)
  ...
  for(GB in 1:3){
    startszout <- lapply(1:4, function(popsz){
      clusterout <- lapply(1:3, function(csz){ # clustered collection number of years
          # GoodBad <- c("random","mostlybad","mildclustered","mostlygood")
          GBmatrix <- list(matrix(c(0.15,0.85,0.15,0.85), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad"))), # random
                           matrix(c(0.45,0.55,0.45,0.55), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad"))), # extreme drought
                           matrix(c(0.55,0.45,0.55,0.45), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad"))), # mild clustered
                           matrix(c(0.85,0.15,0.85,0.15), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad")))) # mostly good
          ...
          noANDharvest <- do.call(rbind,lapply(1:rep_freqint, function(gbreps){# Replicates of different good bad transitions
            # gbtrG2G <- round(runif(1, min = 0, max = 1),2); gbtrB2G <- round(runif(1, min = 0, max = 1),2)
            # gbRANDOM <- matrix(c(gbtrG2G,(1-gbtrG2G),gbtrB2G,(1-gbtrB2G)), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad")))
            # gbNOCOR <- c(0.5,0.5,0.5,0.5)
            # difffromNOCOR <- dist(rbind(as.vector(gbRANDOM), gbNOCOR)) # Euclidean distance 
            # clusterGB <- gbRANDOM[1,1]-gbRANDOM[2,2]
            goodintensity <- runif(1, min = 0, max = 1)
            # Check progress
            print(paste("Replicate",gbreps,"for species:",sp))
          # Keeping total years of the simulation and the good to bad year transition matrix constanst, compare time to exticntion for no harvest
          # compare_sp <- do.call(rbind, apply(Intensityrates, 2, function(i){
          ...
          save(noANDharvest, file = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/GoodIntensity",sp,"popsz",startpop[popsz],"clustersz",clucol[csz],"reps",rep_gb,".Rdata",sep=""))
          noANDharvest
          }) # lapply for how many years of collections
      clusterout
      }) # loop for population starting sizes
