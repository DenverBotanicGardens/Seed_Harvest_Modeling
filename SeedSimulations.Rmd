...
#   })
  ...
  #(1:86,88:97,98:200); 1:length(Mx_all)
  #c(1:97,99:104,106:117)
...
lifelengthsXmatrix[grep("NEMA",names(lifelengthsXmatrix))]
...
TMx_all[grep("NEMA", unlist(Nx_names))]
lifelengthsXspecies[lifelengthsXspecies$SPP == "NEMA",]

avg.lifespan.sp <- aggregate(lifespan~SPP,data = lifelengthsXspecies, median)
...
Arabis fecunda - short lived, no seed bank, no dormancy, fecundity are number of recurits in a stage class by the number of flowering plants the previous year
```{r}
...
lapply(unique(tm$SPP)[1], function(sp){
  ...
  save(bypopsz, file = paste(pathstart, "NoHarvestlistoflists_nodensitydep",sp,sep=""))
  }) # end species

popsz <- c(10,50,100,500)
 
toplotall <- do.call(rbind, lapply(1:4, function(i){
  toplot <- do.call(rbind,lapply(bypopsz[[i]], function(x) x[[2]]))
  toplot$PopSz <- popsz[i]
  toplot
}))
tail(toplotall)

 # ggsave(filename = paste(pathstart,"Noharvest",sp,"_StartPopSz_",popsz,"_.jpg",sep=""),

             ggplot(toplotall, aes(Year, PopulationSize, colour = as.factor(Rep)))+
               geom_line()+
