...
table(tm$SPP)
table(tm$POP)
length(TMx_all[grep("CIPI", names(TMx_all))])
#CIPI get rid of the matrix with different number of stages, do that on its own, most have 3, one population has 6, rename species CIPI3 and CIPI6
...
generationtime <- mapply(function(Mx,Tx){
  Fm <- Mx - Tx
  popbio::generation.time(Mx,  
...
# sp <- unique(tm$SPP)[3]
# rm(sp)
gentimetable <- do.call(rbind,lapply(unique(tm$SPP), function(sp){
  sp <- gsub('[[:digit:]]+', '', sp)
  gentimes <- generationtime[grep(sp, names(generationtime))]
  data.frame(SPP = sp, MeanGenTime = mean(gentimes[!is.infinite(gentimes)], na.rm = TRUE),
             SDGenTime = sd(gentimes[!is.infinite(gentimes)], na.rm = TRUE))
}))

# add annuals after COMPADRE lines 270 on

```

Keyfitz's delta is sum of differences between stable stage distribution (dominant right eigenvector of A) and the provided demographic distribution vector of the population; better might be Cohen's cumulative distance metric
```{r}
library(popdemo)

mapply(function(M,N) Keyfitz.delta(M, N), Mx_all, Nx_all)
KeyfitzD(Mx_all[[1]], Nx_all[[1]])
...
# P plants add to seedbank or next year germinate to new plants - see SEER manuscript outline
...
# Tillandsia brachycaulos is long lived semelparous threatened species
...
generationtimeTIBR <- mapply(function(Mx,Tx){
  Fm <- Mx - Tx
  popbio::generation.time(Mx,  
                          r = c(unique(which(Fm > 0, arr.ind = TRUE)[,1])),
                          c = c(unique(which(Fm > 0, arr.ind = TRUE)[,2])))
}, Mx_all_TIBR[[1]], TMx_all_TIBR[[1]])

mean(generationtimeTIBR)
sd(generationtimeTIBR)


generationtimeAEVI <- mapply(function(Mx,Tx){
  Fm <- Mx - Tx
  popbio::generation.time(Mx,  
                          r = c(unique(which(Fm > 0, arr.ind = TRUE)[,1])),
                          c = c(unique(which(Fm > 0, arr.ind = TRUE)[,2])))
}, Mx_all_AEVI[[1]], TMx_all_AEVI[[1]])

mean(generationtimeAEVI)
sd(generationtimeAEVI)


generationtimeSEER <- mapply(function(Mx,Tx){
  Fm <- Mx - Tx
  popbio::generation.time(Mx,  
                          r = c(unique(which(Fm > 0, arr.ind = TRUE)[,1])),
                          c = c(unique(which(Fm > 0, arr.ind = TRUE)[,2])))
}, Mx_all_SEER, TMx_all_SEER)

mean(generationtimeSEER)
sd(generationtimeSEER)
...
lapply(unique(tm$SPP), function(species){
  Mxs_now <- Mx_all[grep(species, unlist(Nx_names))]
  TMx_now <- TMx_all[grep(species, unlist(Nx_names))] 
  Nx_now <- Nx_all[grep(species, unlist(Nx_names))]
  SimSimple(HarvestType = "None", sppVector = species, FrG = 0, FrB = 0, intG = 0, intB = 0,
            AvgInt = 0, Cgg_gb_bg_bb = c(0.5,0.5,0.5,0.5), reps = 100,
            Mxs = Mxs_now, TMxs = TMx_now, Nxs = Nx_now,
            Simlength = 10, generationspan = FALSE, stablestagestart = TRUE, ps = pathstartempirical)
})
...
lapply(unique(tm$SPP), function(species){
  ...
  SimSimple(HarvestType = "None", sppVector = species, FrG = 0, FrB = 0, intG = 0, intB = 0,
            AvgInt = 0, Cgg_gb_bg_bb = c(0.5,0.5,0.5,0.5), reps = 100,
            Mxs = Mxs_now, TMxs = TMx_now, Nxs = Nx_now,
            Simlength = 3, generationspan = TRUE, stablestagestart = TRUE, ps = pathstartempirical3gen)
})
...
# ---------------------- Figures 3 generations, 10 years, 100 years -------------------
```{r}

# Test
# pathstart <- pathstartVirtual100
# HarvestType <- "NoneVirtual"
# x <- files[[1]]
# rm(pathstart); rm(HarvestType); rm(x)

pullresults <- function(HarvestType, pathstart, SimLength){
  ...
  toplot <- do.call(rbind, lapply(files, function(x){
    ...
    # if(MT != ""){
    out <- data.frame(bypopsz, MatrixType = MT, SimLengthtext = SimLength)
    out
    }))
  toplot
}
...
# pathstartVirtual100 <-  "C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/Simulation_output_virtual/"
# pathstartVirtual3gen <-  "C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/Sim_out_3gen/"
# pathstartVirtual10yrs <-  "C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/Sim_out_10years/"
pathstartVirtual <-  "C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/Sim_outvirtemp/" # even though no empirical yet

# toplot100yrs <- pullresults("NoneVirtual", pathstartVirtual100, "100 Years")
# toplot3gen <- pullresults("None", pathstartVirtual3gen, "3 generations")
# toplot10yrs <- pullresults("None", pathstartVirtual10yrs, "10 years")
toplotvirt10yrs <- pullresults("Virtual10yrs", pathstartVirtual, "10 years")
toplotvirt3gen <- pullresults("Virtual3gen", pathstartVirtual, "3 gen")
toplotvirt100yrs <- pullresults("Virtual100yrs", pathstartVirtual, "100 years")
toplot <- do.call(rbind, list(toplotvirt10yrs,toplotvirt3gen,toplotvirt100yrs))

toplot$SimLengthtext <- factor(toplot$SimLengthtext, levels = c("10 years", "3 gen", "100 years"))
toplot$MatrixType <- factor(toplot$MatrixType, levels = c("Annual","SemelFast","IteroFast","SemelSlow","IteroSlow"))

# How long are the 3 generations compared to 10 and 100 years?
simgenlength1 <- ggplot(toplot[toplot$StPopSz == 500 & toplot$SimLengthtext == "3 gen",], 
       aes(MatrixType, SimLength))+ # , colour = MatrixType))+
  geom_boxplot()+
  theme_bw()+
  ggtitle("a)")+
  ylab("Simulation length\n years")+
  scale_x_discrete(labels = c("Annual" = "Annual",
                               "SemelFast" = "Semelparous\n fast",
                               "IteroFast" = "Iteroparous\n fast",
                               "SemelSlow" = "Semelparous\n slow",
                               "IteroSlow" = "Iteroparous\n slow"))+
  geom_hline(yintercept = 10, colour = "red", linetype="dashed")

simgenlength2 <- ggplot(toplot[toplot$StPopSz == 500 & toplot$SimLengthtext == "3 gen",], 
       aes(MatrixType, generationtime))+ # , colour = MatrixType))+
  geom_boxplot()+
  theme_bw()+
  ggtitle("b)")+
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Virtual3generationspan.jpg",
       simgenlength1 + simgenlength2,
       width=250, height=90,units='mm', dpi=300)

# ----------------- Figure 2 ---------------------
toplot$SimLengthtext <- factor(toplot$SimLengthtext, levels = c("10 years", "3 gen", "100 years"))

# Lambda
stochLam <- ggplot(toplot[toplot$StPopSz == 500,], aes(MatrixType, exp(Tulapprox) , colour = SimLengthtext))+
  geom_boxplot()+
  theme_bw()+
  ...
  scale_colour_manual(name = "Simulation length",
                      ...
                      breaks = c("10 years", "3 gen", "100 years"),
...
# extinction risk
toplotextinctionrisk <- as.data.frame(table(ER = toplot$Exterpated, 
                                            SimLength = toplot$SimLengthtext, 
                                            MatrixType = toplot$MatrixType, 
                                            PopSz = toplot$StPopSz,
                                            GenTime = toplot$generationtime)) 
# to get unique runs, each has unique generation time
toplotextinctionrisk$SimLength <- factor(toplotextinctionrisk$SimLength, 
                                         levels = c("10 years", "3 gen", "100 years"))

ERplot <- ggplot(toplotextinctionrisk[toplotextinctionrisk$ER == 1,], aes(MatrixType, Freq, colour = SimLength))+
  # geom_boxplot()+
...
plant <- name_search("Cypripedium")
pngid <- name_images(uuid = plant[1,])
plantpng <- image_data(pngid$same[[1]]$uid, size = 128)
plantget <- image_get(uuid = plant[2,])
cactus <- image_data(paste(name_search("cactus")[2,]),size = 128)[[1]]
# time to extinction
 ggplot(toplot, aes(SimLengthtext, Time2Ext, colour = as.factor(StPopSz)))+
  # geom_jitter(height = 0, width = 2, alpha = 0.4, shape = 20)+
  # geom_boxplot(x = as.factor(toplot$generationtime[toplot$StPopSz == 500]))+
  # geom_boxplot()+
   geom_point(position = position_jitterdodge())+
  ...
  ggtitle("c) ")+
  facet_wrap(~MatrixType)+
  scale_colour_discrete(name = "Initial\npopulation\nsize")+
   add_phylopic(plantpng, alpha = 0.2)

# Alternative time to extinction
time2extplot <- ggplot(toplot, aes(MatrixType, Time2Ext, colour = SimLengthtext))+
  # geom_boxplot()+
   geom_point(position = position_jitterdodge())+
   geom_violin(scale = "count")+
  theme_bw()+
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Virtualextinctionmetrics.jpg",
       stochLam / (ERplot + time2extplot) +plot_layout(widths = c(2,1), heights = c(1,2)),
       width=250, height=200,units='mm', dpi=300)
```

Figures 10 years, 3 generations, 100 years for empirical data
```{r}
pathstartVirtual3gen # "None" and "AnnualEmpericalNone"
pathstartVirtual10yrs # "None"and "AnnualEmpericalNone"
pathstart100 # "No10years and "No100years and "AnnualEmpericalNone"

toplotEmp3 <- pullresults("None", pathstartVirtual3gen, "3 generations")
toplotEmp3ann <- pullresults("AnnualEmpericalNone", pathstartVirtual3gen, "3 generations")
toplotEmp10 <- pullresults("None", pathstartVirtual10yrs, "10 years")
toplotEmp10ann <- pullresults("AnnualEmpericalNone", pathstartVirtual10yrs, "10 years")
toplotEmp100 <- pullresults("No100years", pathstart100, "100 years")
toplotEmp100annuals <- pullresults("AnnualEmpericalNone", pathstart100, "100 years")

identical(names(toplotEmp3), names(toplotEmp3ann))
identical(names(toplotEmp3), names(toplotEmp100))
names(toplotEmp100) <- names(toplotEmp3)

toplotEmp <- do.call(rbind, list(toplotEmp3,toplotEmp3ann,toplotEmp10,toplotEmp10ann,toplotEmp100,toplotEmp100annuals))

# Label the empirical species by annual, fast and slow itero or semelparous
gentimetable$SPP[gentimetable$SPP == "CIPI"] <- c("CIPI3", "CIPI6")

# Add two annuals
gentimetable <- rbind(gentimetable, c("AEVI",mean(generationtimeAEVI), sd(generationtimeAEVI)))
gentimetable <- rbind(gentimetable, c("SEER",mean(generationtimeSEER), sd(generationtimeSEER)))
gentimetable <- rbind(gentimetable, c("TIBR", mean(generationtimeTIBR),sd(generationtimeTIBR)))

# need to merge with CIPI3 and CIPI6

sp_infotoplot <- merge(sp_infoCIPIone, gentimetable, by = "SPP", all = TRUE)
sp_infotoplot$Iteroparity[sp_infotoplot$MeanGenTime > 5] <- c("Iteroparous slow") 
sp_infotoplot$Iteroparity[sp_infotoplot$MeanGenTime < 5] <- c("Iteroparous fast") 
sp_infotoplot$Iteroparity[sp_infotoplot$SPP == "CIPI"] <- c("Semelparous fast") 
sp_infotoplot$Iteroparity[sp_infotoplot$SPP == "TIBR"] <- c("Semelparous fast") 
sp_infotoplot$Iteroparity[sp_infotoplot$SPP %in% c("AEVI","SEER")] <- "Annual"
toplotEmp1 <- merge(toplotEmp, sp_infotoplot, by = "SPP")

toplotEmp1$Iteroparity <- factor(toplotEmp1$Iteroparity, levels = c("Annual","Semelparous fast","Iteroparous fast","Iteroparous slow"))

toplotEmp2 <- toplotEmp1[!duplicated(toplotEmp1$generationtime),]
table(toplotEmp2$Iteroparity, toplotEmp2$SPP)

simgenlengthEmp1 <- ggplot(toplotEmp2[toplotEmp2$StPopSz == 500 & toplotEmp2$SimLengthtext == "3 generations",], 
       aes(Iteroparity, generationtime*3))+
  geom_boxplot()+
  # geom_jitter()+
  ggtitle("a)")+
  theme_bw()+
...
simgenlength2 <- ggplot(toplotEmp1[toplotEmp1$StPopSz == 500 & 
                                     toplotEmp1$SimLengthtext == "3 generations",], 
       aes(Iteroparity, generationtime))+ # , colour = MatrixType))+
  geom_boxplot()+
  ggtitle("b)")+
  # geom_jitter()+
  theme_bw()+
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Empirical3generationspan.jpg",
       simgenlengthEmp1 + simgenlength2,
       width=250, height=90,units='mm', dpi=300)


# Extinction metrics for empirical data  

toplotEmp1$SimLengthtext <- factor(toplotEmp1$SimLengthtext, levels = c("10 years", "3 generations", "100 years"))
table(toplotEmp1$SPP) # lost CIPI, have 800 of all but AEVI, SEER, and TIBR; still missing 100 years

table(toplotEmp1[toplotEmp1$SPP == "AEVI",]$StPopSz,toplotEmp1[toplotEmp1$SPP == "AEVI",]$SimLengthtext) # 100 for 10 and 3, none for 100 years)
table(toplotEmp1[toplotEmp1$SPP == "TIBR",]$StPopSz,toplotEmp1[toplotEmp1$SPP == "TIBR",]$SimLengthtext) # 100 for 10 and 3, none for 100 years)
table(toplotEmp1[toplotEmp1$SPP == "ARFE",]$StPopSz,toplotEmp1[toplotEmp1$SPP == "ARFE",]$SimLengthtext) # 100 each for 10 and 3, none for 100 


# Lambda
stochLamEmp <- ggplot(toplotEmp1[toplotEmp1$StPopSz == 500,], aes(Iteroparity, exp(Tulapprox) , colour = SimLengthtext))+
  geom_boxplot()+
  theme_bw()+
  ylab(expression(lambda) )+ 
...
# extinction risk
toplotextinctionriskEmp <- as.data.frame(table(ER = toplotEmp1$Exterpated, 
                                            SimLength = toplotEmp1$SimLengthtext, 
                                            MatrixType = toplotEmp1$Iteroparity, 
                                            PopSz = toplotEmp1$StPopSz,
                                            GenTime = toplotEmp1$generationtime)) 
# to get unique runs, each has unique generation time
toplotextinctionriskEmp$SimLength <- factor(toplotextinctionriskEmp$SimLength, 
                                         levels = c("10 years", "3 generations", "100 years"))

ERplotEmp <- ggplot(toplotextinctionriskEmp[toplotextinctionriskEmp$ER == 1,], aes(MatrixType, Freq, colour = SimLength))+  
  geom_point(position = position_jitterdodge())+
  geom_violin()+
  facet_wrap(~PopSz)+
  # coord_flip()+
...
# time to extinction
time2extplotEmp <- ggplot(toplotEmp1, aes(Iteroparity, Time2Ext, colour = SimLengthtext))+
  geom_point(position = position_jitterdodge())+
  geom_violin(scale = "count")+
  theme_bw()+
  scale_colour_manual(name = "Simulation length",
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Empericalextinctionmetrics.jpg",
       stochLamEmp / (ERplotEmp + time2extplotEmp)  +plot_layout(widths = c(2,1), heights = c(1,2)),
       width=250, height=200,units='mm', dpi=300)

```


Fgiures 10 years, 3 generations, 100 years for COMPADRE data   
```{r}
compadre <- pullresults("Virtual10yrs", pathstartVirtual, "10 years")
...
# ---------------------- Figures 100 years vs. 50 generations ------------------------- 
...
stochlambda_all <- ggplot(toplotNo, aes(lifespan, exp(Tulapprox) ))+
  geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
  theme_bw()+
  ylab(expression(lambda))+ # stochastic growth rate \n (Tuljapukar's approximation)")+
  xlab("Generation time")+
  ggtitle("b) Simulation 50 generations")+
  facet_wrap(~StPopSz)
...
# load to bind and plot
HarvestType <- "No100years"
files <- list.files(path = pathstart100, pattern = paste(HarvestType, "Harvest", sep=""))
toplotNo100yrs <- do.call(rbind, lapply(files, function(x){
  load(file = paste(pathstart100,x,sep = ""))
  bypopsz
  }))
stochlambda100yrs <- ggplot(toplotNo100yrs, aes(lifespan, exp(Tulapprox) ))+
  geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
  theme_bw()+
  ylab(expression(lambda) )+ 
  # theme(axis.text.y = element_text(angle = 90))+
  xlab("Generation time")+
  ggtitle("a) Simulation 100 years")+
  facet_wrap(~StPopSz)
  

ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/StochLambdaNoharvest_100yrs50gen.jpg",
       
       stochlambda100yrs +stochlambda_all, 
       
       width=200, height=100,units='mm', dpi=300)


Simlength_100yr_50gen <- rbind(data.frame(SimLength = "100yrs", toplotNo100yrs),
                               data.frame(SimLength = "50gen", toplotNo))

ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/StochLambdaNoharvest_ECDF_100yrs50gen.jpg",
ggplot(Simlength_100yr_50gen, aes(exp(Tulapprox), colour = SimLength))+
  stat_ecdf()+
  facet_wrap(~StPopSz)+
  theme_bw()+
  xlab(expression(lambda) )+ 
  ylab("Empirical cumulative distribution")+
  scale_colour_discrete(name = "Simulation length"), 
width=100, height=80,units='mm', dpi=300)
  

# ----------------- Damping ratio ---------------------
damprat100yrs <- ggplot(toplotNo100yrs, aes(lifespan, DampingRatio ))+
  geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
  theme_bw()+
  ylab(expression(rho) )+ 
  xlab("Generation time")+
...
damprat50gen <- ggplot(toplotNo, aes(lifespan, DampingRatio ))+
  geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
  theme_bw()+
  ylab(expression(rho) )+ 
  xlab("Generation time")+
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/DampingRatio_100yrs50gen.jpg",
       
       damprat100yrs +damprat50gen, 
       
       width=200, height=100,units='mm', dpi=300)


ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/DampingRatio_ECDF_100yrs50gen.jpg",
       ggplot(Simlength_100yr_50gen, aes(DampingRatio,colour = SimLength))+
          stat_ecdf()+
          facet_wrap(~StPopSz)+
          theme_bw()+
          xlab(expression(lambda) )+ 
          ylab("Empirical cumulative distribution")+
          scale_colour_discrete(name = "Damping Ratio"), 
width=100, height=80,units='mm', dpi=300)



# -------------------- Extinction risk --------------------
extinctionrisk100yrs50gen <- aggregate(Exterpated ~ SimLength + SPP + StPopSz + FreqG + FreqB + IntG + IntB + lifespan, 
                                  function(x) sum(x)/length(x), data =  Simlength_100yr_50gen)

extrisk_100yr50gen <- ggplot(extinctionrisk100yrs50gen, aes(lifespan, Exterpated, colour = SimLength))+
  geom_jitter(height = 0, width = 0.5)+
  stat_smooth(method="glm", formula=y~log(x),se=FALSE)+
  theme_bw()+
  xlab("Generation time")+
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/ExtinctionRiskNoharvest_100yrs50gen.jpg",
       extrisk_100yr50gen,
       width=200, height=100,units='mm', dpi=300)
...
# Compare baseline to harvest levels at 100 years and 50 generations   
```{r}
HarvestType <- "SameGB"
files <- list.files(path = pathstart, pattern = paste(HarvestType, "Harvest", sep=""))
...
toplotSame50Gen <- do.call(rbind, lapply(files, function(x){
  load(file = paste(pathstart,x,sep = ""))
  bypopsz
}))

HarvestType <- "SameGB100"
files <- list.files(path = pathstart100, pattern = paste(HarvestType, "Harvest", sep=""))
toplotSame100yrs <- do.call(rbind, lapply(files, function(x){
  load(file = paste(pathstart100,x,sep = ""))
  bypopsz
  }))

table(toplotSame100yrs$FreqG, toplotSame100yrs$IntG)
table(toplotSame50Gen$FreqG, toplotSame50Gen$IntG)

Simlength_100yr_50gen_harvest <- rbind(data.frame(SimLength = "100yrs", toplotSame50Gen),
                               data.frame(SimLength = "50gen", toplotSame100yrs),
                               Simlength_100yr_50gen)

ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/StochLambdaHarvest_100yrs50gen.jpg",
  
  ggplot(Simlength_100yr_50gen_harvest, aes(exp(Tulapprox), colour = as.factor(FreqG), linetype = as.factor(IntG)))+
    stat_ecdf()+
    theme_bw()+
    facet_wrap(~SimLength)+ 
    scale_color_manual(values = c("black","red","orange","darkgreen","blue","purple"),
                       name = "Intensity \n of harvest")+
    scale_linetype_manual(values = c("solid","longdash","dashed","dotted", "dotdash","twodash"),
                          name = "Probability \n of harvest")+
    scale_size_manual(values = c(2,rep(1,5)),guide = "none")+
    scale_alpha_manual(values = c(1,0.95,0.95,0.95,0.95,0.95), guide = "none")+
    ylab("Empirical cumulative distribution")+
    xlab(expression(lambda)),
  width=200, height=100,units='mm', dpi=300)
...
stochlambda_all <- ggplot(toplotNo, aes(lifespan, Tulapprox ))+
         geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
         # stat_smooth(method = "glm")+
         theme_bw()+
         ylab(expression(paste("ln(",lambda,")"), sep=""))+ # stochastic growth rate \n (Tuljapukar's approximation)")+
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/StochLambdaNoharvest.jpg",
       stochlambda_all, width=100, height=100,units='mm', dpi=300)
# 

stochlambda_all_trans <-        ggplot(toplotNo, aes(lifespan, exp(Tulapprox) ))+
         geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
         # stat_smooth(method = "glm")+
         theme_bw()+
         ylab(expression(lambda))+
         xlab("Generation span")+
  ggtitle("a)")
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/StochLambdaNoharvestexp.jpg",
stochlambda_all_trans, width=100, height=100,units='mm', dpi=300)
...
t2ext_all <-        ggplot(toplotNo, aes(lifespan, Time2Ext))+
         # geom_boxplot()+
         theme_bw()+
         facet_wrap(~as.factor(StPopSz))+ #, scales = "free")+
         geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
         ylab("Time to extinction (< 1)")+
         xlab("Generation span")+
    ggtitle("a)")
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Time2ExtNoharvestall.jpg",t2ext_all, 
       width=100, height=100,units='mm', dpi=300)

ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Generations2ExtNoharvest_popsize.jpg",
       ggplot(toplotNo, aes(as.factor(StPopSz), Time2Ext/lifespan, colour = lifespan))+
         geom_jitter(height = 0, width = 0.3, size = 0.5)+ #alpha = 0.85, ,  shape = 16
         # geom_boxplot(alpha = 0.5)+
         theme_bw()+
         ylab("Generations to extinction \n (population < 1)")+
         xlab("Population size")+
         scale_colour_gradient2(low = "green", mid = "blue", high = "orange",
                                midpoint = 40), width=130, height=100,units='mm', dpi=300)


t2ext_loggeneration_all <-  ggplot(toplotNo, aes(as.factor(StPopSz), log(Time2Ext/lifespan), colour = lifespan))+
         geom_jitter(height = 0, width = 0.3, size = 0.5)+ #alpha = 0.85, ,  shape = 16
         # geom_boxplot(alpha = 0.5)+
         theme_bw()+
         ylab("log generations to extinction \n (population < 1)")+
         xlab("Population size")+
         # scale_colour_gradient2(low = "green", mid = "blue", high = "orange",
         #                        midpoint = 40), 
         scale_colour_gradientn(colours = c("cyan","yellow","gold","red","blue"),
                                breaks = c(1,1.01,10,40,86.63166),
                                labels = c(1, "", 10, 40, 80))
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/logGenerations2ExtNoharvest_popsize.jpg",
      t2ext_loggeneration_all,
...
nrow(extinctionrisk[extinctionrisk$Exterpated > 0 & extinctionrisk$StPopSz == 10,]) # 23
nrow(extinctionrisk[extinctionrisk$Exterpated > 0 & extinctionrisk$StPopSz == 50,]) # 20
nrow(extinctionrisk[extinctionrisk$Exterpated > 0 & extinctionrisk$StPopSz == 100,]) # 20
nrow(extinctionrisk[extinctionrisk$Exterpated > 0 & extinctionrisk$StPopSz == 500,]) # 16
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/ExtRiskNoharvest_bysize.jpg",
ggplot(extinctionrisk, aes(lifespan, Exterpated))+
  stat_smooth(method="glm", formula=y~log(x),se=FALSE)+
  theme_bw()+
  facet_wrap(~as.factor(StPopSz))+
...
# ---------------- Does it make a difference if we only do 100 years for each one instead of 50 generations -----------------
Figures pre-harvest conditions
```{r}
...
stochlambda100yrs <-        ggplot(toplotNo100yrs, aes(lifespan, exp(Tulapprox) ))+
  geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
  theme_bw()+
  ylab(expression(lambda) )+ # "Lambda \n (Tuljapukar's approximation transformed)")+
  xlab("Generation span")+
  ggtitle("b)")
  

ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/StochLambdaNoharvestexp100yrs.jpg",
       stochlambda_all_trans  + stochlambda100yrs 
       , width=150, height=75,units='mm', dpi=300)

time2ext100yrs <-        ggplot(toplotNo100yrs, aes(lifespan, Time2Ext))+
         # geom_boxplot()+
         theme_bw()+
         facet_wrap(~as.factor(StPopSz))+ #, scales = "free")+
         geom_jitter(height = 0, width = 0.5, alpha = 0.4, shape = 20)+
         ...
         xlab("Generation span")+
         ggtitle("b)")
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Time2ExtNoharvestall100yrs.jpg",
       t2ext_all + time2ext100yrs, width=200, height=100,units='mm', dpi=300)

ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/Generations2ExtNoharvest_popsize.jpg",
       ggplot(toplotNo100yrs, aes(as.factor(StPopSz), Time2Ext/lifespan, colour = lifespan))+
         geom_jitter(height = 0, width = 0.3, size = 0.5)+ #alpha = 0.85, ,  shape = 16
         # geom_boxplot(alpha = 0.5)+
         theme_bw()+
         ylab("Generations to extinction \n (population < 1)")+
...
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/logGenerations2ExtNoharvest_popsize.jpg",
       ggplot(toplotNo100yrs, aes(as.factor(StPopSz), log(Time2Ext/lifespan), colour = lifespan))+
         geom_jitter(height = 0, width = 0.3, size = 0.5)+ #alpha = 0.85, ,  shape = 16
         # geom_boxplot(alpha = 0.5)+
         theme_bw()+
         ylab("log generations to extinction \n (population < 1)")+
...
extinctionrisk100yrs <- aggregate(Exterpated ~ SPP + StPopSz + FreqG + FreqB + IntG + IntB + lifespan, function(x) sum(x)/length(x) ,data =  toplotNo100yrs)

nrow(extinctionrisk100yrs[extinctionrisk100yrs$Exterpated > 0 & extinctionrisk100yrs$StPopSz == 10,]) # 23
nrow(extinctionrisk100yrs[extinctionrisk100yrs$Exterpated > 0 & extinctionrisk100yrs$StPopSz == 50,]) # 20
nrow(extinctionrisk100yrs[extinctionrisk100yrs$Exterpated > 0 & extinctionrisk100yrs$StPopSz == 100,]) # 20
nrow(extinctionrisk100yrs[extinctionrisk100yrs$Exterpated > 0 & extinctionrisk100yrs$StPopSz == 500,]) # 16



ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/ExtRiskNoharvest.jpg",
ggplot(extinctionrisk100yrs, aes(lifespan, Exterpated))+
  # geom_smooth(method=MASS::glm.nb, se = FALSE)+
  stat_smooth(method="glm", formula=y~log(x),se=FALSE)+
  theme_bw()+
  facet_wrap(~as.factor(StPopSz))+
...
# Biased in good or bad, equal an average   
```{r}
pathstart <- "C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/Simulation_output/"
...
toplot$IntFreq <- paste(toplot$IntG,toplot$FreqG,sep = "_")
unique(toplot$IntFreq)


# --------------------------------- cumulative distributions ----------------------
ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/StochLambdaCDF.jpg",
  ggplot(toplot, aes(Tulapprox, colour = as.factor(FreqG), linetype  = as.factor(IntG),
                     size = as.factor(IntG), alpha = as.factor(IntG)))+
    stat_ecdf()+
    facet_wrap(~LifeHistory, ncol = 1)+
    theme_bw()+
    scale_color_manual(values = c("black","red","orange","darkgreen","blue"),
                       name = "Intensity \n of harvest")+
    scale_linetype_manual(values = c("solid","longdash","dashed","dotted", "dotdash","twodash"),
                          name = "Probability \n of harvest")+
    scale_size_manual(values = c(2,rep(1,5)),guide = "none")+
    scale_alpha_manual(values = c(1,0.95,0.95,0.95,0.95,0.95), guide = "none")+
    ylab("Empirical cumulative distribution")+
    xlab("Stochastic Lambda \n (simulations of 50 generations)"),
width=175, height=150,units='mm', dpi=300)
  

ggsave(filename = "C:/Users/DePrengm/OneDrive - Denver Botanic Gardens/P drive/My Documents/UCDenver_phd/Dissertation/Chapter3/Figures/StochLambdaCDFall.jpg",
  ggplot(toplot, aes(exp(Tulapprox), colour = as.factor(FreqG), linetype  = as.factor(IntG),
                     size = as.factor(IntG), alpha = as.factor(IntG)))+
    stat_ecdf()+
    theme_bw()+
    scale_color_manual(values = c("black","red","orange","darkgreen","blue"),
...
ksresulttable <- do.call(rbind,lapply(c(0.1,0.25,0.5,0.75, 0.9), function(i){
  fout <- do.call(rbind,lapply(c(0.1,0.25,0.5,0.75), function(f){
    testout <- ks.test(toplot$Tulapprox[toplot$FreqG==0 & toplot$IntG == 0],
        toplot$Tulapprox[toplot$FreqG==f & toplot$IntG == i],
        alternative = "less")
    dfout <- data.frame(Freq = f, Int = i, D = testout$statistic, p = testout$p.value)
  }))
  fout
}))

ksresulttableannuals <- do.call(rbind,lapply(c(0.1,0.25,0.5,0.75, 0.9), function(i){
  fout <- do.call(rbind,lapply(c(0.1,0.25,0.5,0.75), function(f){
      testout <- ks.test(toplot$Tulapprox[toplot$FreqG==0 & toplot$IntG == 0 & toplot$lifespan == 1],
                         toplot$Tulapprox[toplot$FreqG==f & toplot$IntG == i & toplot$lifespan == 1],
                         alternative = "less")
      dfout <- data.frame(Freq = f, Int = i, D = testout$statistic, p = testout$p.value)
      }))
  fout
...
ksresulttableshort <- do.call(rbind,lapply(c(0.1,0.25,0.5,0.75, 0.9), function(i){
  fout <- do.call(rbind,lapply(c(0.1,0.25,0.5,0.75), function(f){
      testout <- ks.test(toplot$Tulapprox[toplot$FreqG==0 & toplot$IntG == 0 & toplot$lifespan > 1& toplot$lifespan < 11],
                         toplot$Tulapprox[toplot$FreqG==f & toplot$IntG == i & toplot$lifespan > 1& toplot$lifespan < 11],
                         alternative = "less")
      dfout <- data.frame(Freq = f, Int = i, D = testout$statistic, p = testout$p.value)
      }))
  fout
...
ksresulttableslong <- do.call(rbind,lapply(c(0.1,0.25,0.5,0.75, 0.9), function(i){
  fout <- do.call(rbind,lapply(c(0.1,0.25,0.5,0.75), function(f){
      testout <- ks.test(toplot$Tulapprox[toplot$FreqG==0 & toplot$IntG == 0 & toplot$lifespan > 9],
                         toplot$Tulapprox[toplot$FreqG==f & toplot$IntG == i & toplot$lifespan > 9],
                         alternative = "less")
      dfout <- data.frame(Freq = f, Int = i, D = testout$statistic, p = testout$p.value)
      }))
  fout
...
nrow(lifespan.sp[lifespan.sp$generationTime < 10,]) # 12
nrow(lifespan.sp[lifespan.sp$generationTime >9,]) # 10


test1 <- ks.test(toplot$Tulapprox[toplot$FreqG==0 & toplot$IntG == 0],
        toplot$Tulapprox[toplot$FreqG==0.9 & toplot$IntG == 0.75],
        alternative = "less")
test1$statistic

## Can I do a bifurcating analysis to classify PDFs (or cumulative distribution curves) into groups and see if annuals and some split of lifespan comes out? 
...
save(toplot, file = paste(pathstart,"DF_toplot_equal.Rdata", sep=""))
...
# ------------------------------------ Frequency or Intensity, which matters more? ------------------

require(AICcmodavg)

# offset for intercept for either every spp is different or differs by generation time 
lm1 <- lm(Tulapprox  ~ IntG + FreqG + SPP, data = toplot)
lm2 <- lm( Tulapprox ~ IntG + FreqG + LifeHistory, data = toplot)
lm3 <- lm( Tulapprox ~ IntG + SPP, data = toplot)
lm4 <- lm( Tulapprox ~ FreqG + SPP, data = toplot)
lm5 <- lm( Tulapprox ~ IntG + LifeHistory, data = toplot)
lm6 <- lm( Tulapprox ~ FreqG + LifeHistory, data = toplot)
lm7 <- lm( Tulapprox ~ FreqG + IntG + lifespan, data = toplot) # Where lifespan is really generation time from
lm8 <- lm( Tulapprox ~ IntG + lifespan, data = toplot) # Where lifespan is really generation time from
lm9 <- lm( Tulapprox ~ FreqG + lifespan, data = toplot) # Where lifespan is really generation time from popbio::generation.time
# different slopes and intercepts?
lm10 <- lm( Tulapprox ~ (IntG + FreqG) * SPP, data = toplot)
lm11 <- lm( Tulapprox ~ (IntG + FreqG) * LifeHistory, data = toplot)
lm12 <- lm( Tulapprox ~ IntG * SPP, data = toplot)
lm13 <- lm( Tulapprox ~ FreqG * SPP, data = toplot)
lm14 <- lm( Tulapprox ~ IntG * LifeHistory, data = toplot)
lm15 <- lm( Tulapprox ~ FreqG * LifeHistory, data = toplot)
lm16 <- lm( Tulapprox ~ (FreqG + IntG) * lifespan, data = toplot) # Where lifespan is really generation time from
lm17 <- lm( Tulapprox ~ IntG * lifespan, data = toplot) # Where lifespan is really generation time from
lm18 <- lm( Tulapprox ~ FreqG * lifespan, data = toplot) # Where lifespan is really generation time from popbio::generation.time

lm.list <- list(lm1,lm2,lm3,lm4,lm5,lm6,lm7,lm8,lm9,lm10,lm11,lm12,lm13,lm14,lm15,lm16,lm17,lm18)
# lm.list <- do.call(as.name,list(paste("lm",1:18,sep="")))
lm.names <- as.character(unlist(lapply(lm.list, formula)))
(lm.results <- aictab(lm.list, modnames = lm.names))
...
# Which describes different more, frequency or intensity?   
```{r}
head(toplot)
head(extinctionrisk)
...
some are "OnlyGood", "OnlyGB", and some "Same" do not have equal good and bad intensity and frequencies, need to cut out wrong ones (nervous to delete them!)
