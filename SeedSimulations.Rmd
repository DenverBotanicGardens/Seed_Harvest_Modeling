...
harvestsims <- lapply(startpop, function(s){
  clusters <- lapply(clucol[1], function(clust){
    intensity <- lapply(seq(0.1,1,by=0.1), function(i){ # Average intensity
      resultpath <- list.files(path = pathstart, 
                               pattern = paste("popsz",s,"AvgIntensity",i,"clustersz",clust,"GB2","reps100",sep=""), 
                               full.names=TRUE)
      cl <- makeCluster(cores)
      registerDoParallel(cl)
      diff.discrete <- foreach(x = resultpath, .combine = "rbind") %:% 
        foreach(harv = levels(factor(noANDharvest$Harvest))[levels(factor(noANDharvest$Harvest))!="0:0:0:0"], .combine = "rbind") %dopar% {
          load(x) # loads a noANDharvest simulation for a species/clustersize/popsize/variableGBs
          
          if(any(!is.na(noANDharvest$Yr2Ext[noANDharvest$Harvest=="0:0:0:0"])) &
               any(!is.na(noANDharvest$Yr2Ext[noANDharvest$Harvest==harv]))){
              diff.Yr2Ex <- do.call(rbind,lapply(1:10, function(reps){
                # eventually check across levels of harvest, now is levels of good to bad
                ...
                har_idx <- sample(1:nrow(harvest), size = 1)
                out <- data.frame(DiffTime2Ext = harvest$Yr2Ext[har_idx]/sample(noharvest$Yr2Ext, size = 1),
          ...
          } # end dopar, spit out diff.discrete
