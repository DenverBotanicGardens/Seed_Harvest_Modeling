...
Mx_list <- Mx_all[grep(sp, unlist(Nx_names))]
...
# ceil <- 10*
  ...
  # difference between the largest and smallest population size observed  
# the most is 95 times larger the max compared to the min pop size, these are only observable plants
for(i in  unique(tm$SPP)){
  Nx_list <-  Nx_all[grep(i, unlist(Nx_names))]
  hist(unlist(lapply(Nx_list, function(x) sum(x, na.rm = TRUE))) , breaks=30 )
  print(max(unlist(lapply(Nx_list, function(x) sum(x, na.rm = TRUE))))/min(unlist(lapply(Nx_list, function(x) sum(x, na.rm = TRUE)))))
}
    # vec <- matrix(floor(Nx*(StartPopSize/sum(Nx))), ncol = 1)
# # 
...
PHIN 2      
PHEM 2      
TRGR 1 - but the first is germinants that are estimated with destructive sampling of seed baskets 
CIPI6 1

```{r}


# Enter plants as seedlings, not as seed
...
apply(Nx_all, 1, function(x) which(is.na(x[[1]])))
...
SimSimple <- function(HarvestType = "No", sppVector = unique(tm$SPP)[-2], FrG = 0, FrB = 0, intG = 0, intB = 0, 
  ...
  lapply(sppVector, function(sp){
    bypopsz <- do.call(rbind,lapply(c(10,50,100,500), function(popsz){
      outdf <- do.call(rbind,lapply(1:reps, function(rep){
        ...
        out <- data.frame(Tulapprox = outSimulation$LogGrowthRate$approx, 
                          ...
                          FreqG = FrG, FreqB = FrB, 
                          IntG = intG, IntB = intB,
                          SPP = sp, Clust = 1, StPopSz = popsz, lifespan = floor(lifespan.sp$Lifespan[lifespan.sp$SPP==sp]),
  ...
  # bypopsz
  save(bypopsz, file = paste(pathstart, HarvestType,"Harvest",sp,
                             "Fr",FrG,"_",FrB,"In",intG,"_",intB,"AverageInt",AvgInt,
                             "Climate",paste(Cgg_gb_bg_bb, collapse = "_"),
                             ".Rdata", sep=""))
  }) # end species
...
SimSimple(HarvestType = "No", sppVector = c("CIPI3","CIPI6"))

# load to bind and plot
toplot <- do.call(rbind, lapply(unique(tm$SPP)[1], function(sp){
  load(paste(pathstart, HarvestType,"Harvest", sp, ".Rdata", sep=""))
  bypopsz
  }))

ggplot(toplot, aes(as.factor(StPopSz), DampingRatio, colour = SPP))+
  geom_boxplot()+
  theme_bw()+
  geom_jitter(height=0, width = 0.1)
  # facet_wrap(~PopSz, scales = "free")+
  #              theme(legend.position="none",
...
#CIPI get rid of the matrix with different number of stages, do that on its own, most have 3, one population has 6
TMx_all[grep("CIPI", unlist(Nx_names))][which(sapply(TMx_all[grep("CIPI", unlist(Nx_names))], function(x) dim(x))[1,]==3)]

lapply(unique(tm$SPP)[-2], function(sp){
  
  # Frequency of collections in good years
  
  bypopsz <- do.call(rbind,lapply(c(10,50,100,500), function(popsz){
  # lapply(0, function(FrG){
    # avgInt <- lapply(0, function(avgInt){
      outdf <- do.call(rbind,lapply(1:100, function(rep){
        outSimulation <- SeedHarvestSim(Mx_list = Mx_all[grep(sp, unlist(Nx_names))], 
                                        TMx_list = TMx_all[grep(sp, unlist(Nx_names))],
                                        Nx_list = Nx_all[grep(sp, unlist(Nx_names))],
                                        Nx = stable.stage(mean(Mx_all[grep(sp, unlist(Nx_names))])),
                                        ...
                                        ClusteredColl = 1, # clust,
                                        ddceiling = FALSE)
        save(outSimulation, file = paste(pathstart,"GoodYrHarvest_",sp,"Freq",FrG,"Int",intG,"PopSz",popsz,"Rep",rep,".Rdata", sep="")) #"AvgInt",avgInt,"Freq",FrG,"
  ...
  # bypopsz
  save(bypopsz, file = paste(pathstart, "GoodYrHarvest",sp,"Freq",FrG,"Int",intG,".Rdata", sep=""))
  }) # end species

# load to bind and plot
toplot <- do.call(rbind, lapply(unique(tm$SPP)[1], function(sp){
  load(paste(pathstart, "NoHarvest", sp, ".Rdata", sep=""))
  bypopsz
  }))
...
# the following use the simulation function with errors - delete after next commit
Constrain
