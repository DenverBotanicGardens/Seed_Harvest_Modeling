...
SeedHarvestSim <- function(Mx_list, TMx_list, Nx, StartPopSize, GoodBadTm, Freq = c(0,0), Int = c(0,0), TotYrs, ClusteredColl = 1, ddceiling = FALSE){
  ...
  # Start with good; fill vectors to hold distribution of good or bad years, the frequency at which a year is harvested (CollectYN), the intensity or percentage fecundity is reduced selected from the good or bad rates (depending on which it is); and the population size which is saved in list 
...
sp <- unique(tm$SPP)[3]
out <- do.call(rbind, lapply(1:100, function(x){
