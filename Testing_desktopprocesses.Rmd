...
SeedHarvestSim <- function(Mx_list, TMx_list, Nx, StartPopSize, GoodBadTm, Freq = c(0,0), Int = c(0,0), 
  ...
  if(ClusteredColl > 1){
    ...
    collectionyr_ix <- c()
    while(i < numclusters){
      available <- setdiff((1:(TotYrs-ClusteredColl)),collectionyr_ix) # but need to substract ClusteredColl from each start (clst) 
      clst <- sample(size = 1, x = available) # select from possible non-overlapping starts
      testclust <- c(clst:(clst+ClusteredColl-1))
      # if(any(diff(c(1,collectionyr_ix,TotYrs)) >ClusteredColl)){ # is there space for another non-overlapping cluster?
