...
for(sp in unique(tm$SPP)[3]){ # [-2]CIPI has one population with different number of stages
  # for(GB in 1:4){
    for(popsz in 1:1){
      for(csz in 1:1){ # clustered collection number of years
          ...
          noANDharvest <- do.call(rbind,lapply(1:100, function(gbreps){# Replicates of different good bad transitions
          ...
          if(!is.na(any(noANDharvest$Yr2Ext))){ # Does the population evenutally go to extinction?
          ...
          } else {  # if(is.na(any(noharvest$Yr2Ext))){ # Some populations do not go extinct
          ...
          ggsave(filename = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/","_StartPopSz_",startpop[popsz],"clustersz",clucol[csz],"_",sp,"_diffEndPopSzXperiodicity.jpg",sep=""),
                 ggplot(diff.time2ext[diff.time2ext$Harvest == harvlev,], aes(DistGB, DiffTime2Ext))+
                   ...
                   labs(title = paste(sp,"Time to extinction at harvest level:",harvlev),
                        x = "Distance of year transition matrix from random",
