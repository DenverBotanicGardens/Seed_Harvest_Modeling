...
# rm(GoodBadTm); rm(Freq); rm(Int); rm(Mx_list); rm(TMx_list); rm(Nx); rm(StartPopSize); rm(TotYrs); rm(ClusteredColl); rm(seedbank); rm(fecundityRows)
...
# TotYrs/ClusteredColl # need to collect; I divide the percent chance by the cluster
# What is different if pre- or post-reproductive census??
# Nx should be stable stage distribution;

#Takes all objects from the SeedHarvestSim environment
AnnualTransition <-function(){
  ...
  if(CollectYN == 1){  # yes collect, reduce fecuntity by the year good or bad
    if(seedbank == TRUE){
      nx[fecundityRows,-1] <- nx[fecundityRows,-1] - nx[fecundityRows,-1]*Inten_m[Yr,] 
      } else {
        nx[fecundityRows,] <- nx[fecundityRows,-1] - nx[fecundityRows,]*Inten_m[Yr,] 
      }
    }
...
SeedHarvestSim <- function(Mx_list, TMx_list, Nx, StartPopSize, GoodBadTm, Freq = c(0,0), Int = c(0,0), TotYrs, ClusteredColl = 1, fecundityRows = c(1), seedbank = FALSE){
  ...
  while(yrs < TotYrs){
    ...
    vec <- onetransition["vec"][[1]]
    PopSize[yrs] <- floor(sum(onetransition["vec"][[1]]))
    ...
    if(Freqs[yrs-1] == 1){
      ...
      while(clust < ClusteredColl){
          ...
          vec <- onetransitionCluster["vec"][[1]]
          PopSize[yrs] <- floor(sum(onetransitionCluster["vec"][[1]]))
          # Transition between good and bad years
          Yr <- if(rbinom(1,1,GoodBadTm[Yr,1])==1){
            "Good"
          } else {
