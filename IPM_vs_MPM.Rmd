...
Constrain
```{r}
GBmatrix <- matrix(c(0.5,0.5,0.45,0.55), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad")))
...
FreqRates <- cbind(rbind(seq(0,1,by=0.1),seq(0,1,by=0.1)))
sp <- unique(tm$SPP)[5]
...
harvestfreq.int <- do.call(rbind, apply(FreqRates, 2, function(f){
      Intout <- do.call(rbind, apply(Intensityrates, 2, function(i){
        nowharvest <- do.call(rbind, lapply(1:100, function(x){
          out_rep <- SeedHarvestSim(Mx_list = Mx_all[grep(sp, unlist(Nx_names))], 
                                    TMx_list = TMx_all[grep(sp, unlist(Nx_names))],
                                    Nx = stable.stage(mean(Mx_all[grep(sp, unlist(Nx_names))])),
                                    StartPopSize = 500,
  ...
  out.freq <- do.call(rbind,lapply(split(y, y$GoodInt), function(x){
    out.good <- do.call(rbind,lapply(split(x, x$BadInt), function(x1){
      out.bad <- do.call(rbind,lapply(split(x1, x1$Replicate), function(x2){
        Time2Exc <- which.max(x2$PopulationSize<1) # To get the first year the population falls below 1
        # which.max(x$PopulationSize<1) # To get the first year the population falls below 1
...
# clusterExport(clust = clust, varlist=c(""), envir = environment()) # can also import packages

time2ext.df <- as.data.frame(do.call(rbind, lapply(time2extinct, as.vector)))
# time2ext.df <- as.data.frame(do.call(rbind,time2extinct))
names(time2ext.df) <- c("Years", "GoodInt", "BadInt", "GoodFreq", "BadFreq", "Replicate")
...
time2ext.df <-rbind(time2ext.df.noharvest, time2ext.df)
time2ext.df$Harvest <- interaction(time2ext.df$GoodFreq, time2ext.df$BadFreq, time2ext.df$GoodInt, time2ext.df$BadInt, sep=":")
...
GBmatrix <- matrix(c(0.45,0.55,0.35,0.65), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Good","Bad")))
# Intensityrates <- cbind(rbind(seq(0.1,1,by=0.1),seq(0.1,1,by=0.1)),combn(seq(0,1,by=0.1),2))
#What if all the same, no difference between good and bad
Intensityrates <- cbind(rbind(seq(0.1,1,by=0.1),seq(0.1,1,by=0.1)))
FreqRates <- cbind(rbind(seq(0,1,by=0.1),seq(0,1,by=0.1)))
sp <- unique(tm$SPP)[5]
...
# ggplot(noharvest, aes(Year, PopulationSize, colour = as.factor(Replicate)))+
#   geom_line()+
#   theme(legend.position="none")

harvestfreq.int <- do.call(rbind, apply(Intensityrates, 2, function(i){
    nowharvest <- do.call(rbind, lapply(1:100, function(x){
      out_rep <- SeedHarvestSim(Mx_list = Mx_all[grep(sp, unlist(Nx_names))], 
                                TMx_list = TMx_all[grep(sp, unlist(Nx_names))],
                                Nx = stable.stage(mean(Mx_all[grep(sp, unlist(Nx_names))])),
      ...
      out_rep[[1]]$Replicate <- x
      out_rep[[1]]$Year <- 1:100
      out_rep[[1]]$GoodFreq <- 0.2
      out_rep[[1]]$BadFreq <- 0.2
      out_rep[[1]]$GoodInt <- i[1]
      out_rep[[1]]$BadInt <- i[2]
      out_rep[[1]]
      }))
...
ggplot(harvestfreq.int, aes(Year, PopulationSize, colour = as.factor(Replicate)))+
  geom_line()+
  theme(legend.position = "none")+
  facet_wrap(~GoodInt + BadInt)

time2extinct <- sapply(split(harvestfreq.int, harvestfreq.int$GoodInt), function(x){
  out.good <- lapply(split(x, x$BadInt), function(x1){
    out.bad <- do.call(rbind,lapply(split(x1, x1$Replicate), function(x2){
      Time2Exc <- which.max(x2$PopulationSize<1) # To get the first year the population falls below 1
      # which.max(x$PopulationSize<1) # To get the first year the population falls below 1
      GoodInt <- unique(x2$GoodInt)
...
length(time2extinct) # 10 for each of the intensities 

colors.10 = c(rgb(0,0,0,1), rgb(1,1,0,.25), rgb(1,0,1,.25), rgb(.1,0,0.5,.25), rgb(.1,0,0.50,.25),
              rgb(1,0.5,0,.25), rgb(0,0.75,0,.25), rgb(1,0,0,.25), rgb(0,1,0,.25), rgb(1,1,1,.25))


plot(density(time2extinct.noharvest), col = "red", xlim = c(0,70), ylim = c(0,.075))
polygon(density(time2extinct.noharvest), col = "red")
for(x in 1:10){
  polygon(density(time2extinct[[x]][,1]), col = colors.10[x])
}
polygon(density(time2extinct.noharvest), col = rgb(1,0,0,0.75))
