...
TMx_all[[100]];Mx_all[[100]];Nx_all[[100]]

# I think they need to be transposed for the top row to be fecundity
colSums(t(Mx_all[[1]]));colSums(Mx_all[[1]])

lambdas_transp <- sapply(Mx_all, function(x) lambda(t(x)), simplify = FALSE, USE.NAMES = TRUE)
lambdas <- sapply(Mx_all, function(x) lambda(x), simplify = FALSE, USE.NAMES = TRUE)

lambs <- data.frame(unlist(lambdas),unlist(lambdas_transp))
```
