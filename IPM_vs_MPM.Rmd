...
SeedHarvestSim <- function(Mx_list, GoodBadTm, FreqTm, IntTm){
  # lapply(unique(tm$SPP)[-2], function(sp){
  # get transition with fecundity matrices
  Mx_sp <- Mx_all[grep(sp, unlist(Nx_names))]
  # if Lambda is the cutoff for good vs. bad year
  l_all <- lambdas[grep(sp, unlist(Nx_names))] # All annual transition lambdas by site and year
  mn_l <- mean(unlist(l_all))
  above_l <- Mx_sp[which(l_all >= mn_l)]
  below_l <- Mx_sp[which(l_all < mn_l)]
  ...
  f_all <- fecundity[grep(sp, unlist(Nx_names))]
  mn_f <- mean(unlist(f_all)) 
  above_f <- Mx_sp[which(f_all >= mn_f)]
  below_f <- Mx_sp[which(f_all < mn_f)]
})
