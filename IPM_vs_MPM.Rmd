...
Mx_list <- Mx_all[grep("PHEM", unlist(Nx_names))]
TMx_list <- TMx_all[grep("PHEM", unlist(Nx_names))]
Nx <- stable.stage(mean(Mx_all[grep("PHEM", unlist(Nx_names))]))
StartPopSize <- 500
TotYrs <- 100
ClusteredColl <- 4
# seedbank <- TRUE
...
rm(GoodBadTm); rm(Freq); rm(Int); rm(Mx_list); rm(TMx_list); rm(Nx); rm(StartPopSize); rm(TotYrs); rm(ClusteredColl)
...
ggplot(compare_sp[compare_sp$GoodFreq == 0.9 & compare_sp$BadFreq == 0.9 & compare_sp$GoodInt == 0.9,], aes(Year, PopulationSize, colour = as.factor(Replicate)))+
  geom_line()+
  theme(legend.position="none")+
  # geom_rug(colour = as.numeric(compare_sp$GB_yrs[compare_sp$GoodFreq == 0.1 & compare_sp$BadFreq == 0.1]), sides="b")+
  facet_wrap(~as.factor(BadInt))
