...
for(sp in unique(tm$SPP)[-2]){ # [-2]CIPI has one population with different number of stages
  # for(GB in 1:4){
    for(popsz in 1:3){
      for(csz in 1:3){ # clustered collection number of years
          ...
          noANDharvest <- do.call(rbind,lapply(1:10, function(gbreps){# Replicates of different good bad transitions
          ...
          # compare_sp <- do.call(rbind, apply(Intensityrates, 2, function(i){
              ...
              noharvest <- do.call(rbind, lapply(1:100, function(x){ # replicates
                ...
                out_n <- data.frame(EndPopSz = out_rep[[1]]$PopulationSize[nrow(out_rep[[1]])],  # End population size
                    Replicate = x,GoodFreq = 0,BadFreq = 0,GoodInt = 0, BadInt = 0, 
                    Harvest = paste(0, 0, 0, 0, sep=":"),
                    GB_mat = paste(as.vector(gbRANDOM), collapse = ":"),
                    DiffGB_mat = difffromNOCOR[1], Lifespan = floor(lifespan.sp$lifespan[lifespan.sp$SPP==sp]), 
          ...
          save(noANDharvest, file = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/noANDharvest",sp,"popsz",startpop[popsz],"clustersz",clucol[csz],".Rdata",sep=""))
          # noharvest
          # harvestfreq.int
          # together are noANDharvest

          harvestlevels <- levels(factor(noANDharvest$Harvest))
          if(any(!is.na(noANDharvest$Yr2Ext[noANDharvest$Harvest == "0:0:0:0"]))){ # Does the population evenutally go to extinction?
            diff.time2ext <- do.call(rbind,lapply(harvestlevels[harvestlevels!="0:0:0:0"] ,function(harv){ # how many columns of frequency and intensity rates
              diff.freq <- do.call(rbind,lapply(1:1000, function(x){
                ...
                if(nrow(noharvest)==0) break
    print(paste("Number of no harvest years to extinction", nrow(noharvest)))
                harvest <- noANDharvest[noANDharvest$Harvest==harv & !is.na(noANDharvest$Yr2Ext),]
                if(nrow(harvest)==0) break
    print(paste("Number of harvest years to extinction", nrow(harvest)))
                har_idx <- sample(1:(nrow(harvest)), size = 1)
                out <- data.frame(DiffTime2Ext = (sample(noharvest$Yr2Ext, size = 1)-(harvest$Yr2Ext[har_idx])), 
          ...
          if(any(is.na(noANDharvest$Yr2Ext))){ # Some populations do not go extinct
            diff.endpopsz <- do.call(rbind,lapply(harvestlevels[harvestlevels!="0:0:0:0"] ,function(harv){ # how many columns of frequency and intensity rates
              diff.freq <- do.call(rbind,lapply(1:1000, function(x){
                ...
                if(nrow(harvest)==0) break
                har_idx <- sample(1:nrow(harvest), size = 1)
                out <- data.frame(DiffEndPopSz = (log(sample(noharvest$EndPopSz, size = 1))-(log(harvest$EndPopSz[har_idx]))), 
                                  GB_matrix = harvest$GB_mat[har_idx], DistGB = harvest$DiffGB_mat[har_idx],
                                  Harvest = harv, lifespan = floor(lifespan.sp$lifespan[lifespan.sp$SPP==sp]))
                out
                }))
...
for(harvlev in harvestlevels[harvestlevels!="0:0:0:0"]){
  ...
  if(exists("diff.time2ext")){
    ggsave(filename = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/","_StartPopSz_",startpop[popsz],"clustersz",clucol[csz],"_",sp,"_diffEndPopSzXperiodicity.jpg",sep=""),
           ggplot(diff.time2ext[diff.time2ext$Harvest == harvlev,], aes(DistGB, DiffTime2Ext))+
             ...
             labs(title = paste(sp,"Time to extinction at harvest level:",harvlev),
                  x = "Distance of Good:Bad matrix to random",
                  y = "Difference from no harvest"),
           width=300, height=300,units='mm', dpi=300)
  }
  if(exists("diff.endpopsz")){
    # difference from 100% being no harvest; #,GoodBad[GB]
    ggsave(filename = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/","_StartPopSz_",startpop[popsz],"clustersz",clucol[csz],"_",sp,"_difftime2extXperiodicity.jpg",sep=""),
      ...
      rm(diff.time2ext);rm(diff.endpopsz)
      } # Harvest levels

      } # loop for how many years of collections
