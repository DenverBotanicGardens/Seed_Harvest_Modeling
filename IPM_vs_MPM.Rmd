...
```{r}
for(sp in unique(tm$SPP)[1]){ # [-2]CIPI has one population with different number of stages
  # for(GB in 1:4){
    for(popsz in 1:4){
      for(csz in 1:3){
          ...
          totyr <- 100*(floor(lifespan.sp$lifespan[lifespan.sp$SPP==sp])) # make it 100*lifespan
          startpop <- c(100,500,1000,10000)
          # clucol <- c(1,3,5)
          clucol <- 1
          
          noANDharvest <- do.call(rbind,lapply(1:500, function(gbreps){# Replicates of different good bad transitions
          ...
          # compare_sp <- do.call(rbind, apply(Intensityrates, 2, function(i){
              ...
              noharvest <- do.call(rbind, lapply(1:10, function(x){
                out_rep <- SeedHarvestSim(Mx_list = Mx_all[grep(sp, unlist(Nx_names))], 
                                          TMx_list = TMx_all[grep(sp, unlist(Nx_names))],
                                          Nx = stable.stage(mean(Mx_all[grep(sp, unlist(Nx_names))])),
                                          StartPopSize = startpop[popsz],
                ...
                out_n <- data.frame(EndPopSz = out_rep[[1]]$PopulationSize[nrow(out_rep[[1]])],  # End population size
                  ...
                  Yr2Ext = which(out_rep[[1]]$PopulationSize<1)[1]) # Time to extinction: the first year the population falls below 1; when never met which.max returns 1
                out_n
                }))

            harvestfreq.int <- do.call(rbind,apply(FreqRates, 2, function(f){
              nowharvest <- do.call(rbind,lapply(1:1, function(x){ # without parallel; replicates of frequency
                Intout <- do.call(rbind,lapply(1:10, function(irep){ # make 1:1000 after it works, selecting new intensities
          ...
          if(!is.na(any(noANDharvest$Yr2Ext))){ # Does the population evenutally go to extinction?
            diff.time2ext <- do.call(rbind, lapply(harvestlevels[harvestlevels!="0:0:0:0"] ,function(harv){ # how many columns of frequency rates
          ...
          if(is.na(any(noharvest$Yr2Ext))){ # Some populations do not go extinct
            diff.endpopsz <- do.call(rbind,lapply(harvestlevels[harvestlevels!="0:0:0:0"] ,function(harv){ # how many columns of frequency rates
              diff.freq <- do.call(rbind,lapply(1:1000, function(x){
                # eventually check across levels of harvest, now is levels of good to bad
                ...
                har_idx <- sample(1:nrow(harvest), size = 1)
                out <- data.frame(DiffTime2Ext = (sample(noharvest$EndPopSz, size = 1)-(harvest$EndPopSz[har_idx])), 
                                  GB_matrix = harvest$GB_mat[har_idx], DistGB = harvest$DiffGB_mat[har_idx])
                out
                }))
              diff.freq
          ...
          ggsave(filename = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/",GoodBad[GB],"_StartPopSz_",startpop[popsz],"_",sp,"_diffEndPopSzXperiodicity.jpg",sep=""),
                 ggplot(diff.time2ext, aes(DistGB, DiffTime2Ext))+
                   geom_point()+
                   theme(legend.position="none",
                         axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
                         panel.background = element_rect(fill = "white", colour = "grey75"))+
          ...
          # difference from 100% being no harvest
          ggsave(filename = paste("C:/Users/DePrengm/Denver Botanic Gardens/Conservation - Demography/SeedHarvestModelling/Seed_Harvest_Modeling/",GoodBad[GB],"_StartPopSz_",startpop[popsz],"_",sp,"_difftime2extXperiodicity.jpg",sep=""),
                 ggplot(diff.time2ext, aes(Harvest, DiffTime2Ext))+
                   geom_boxplot()+
                   theme(legend.position="none",
                   axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
                   panel.background = element_rect(fill = "white", colour = "grey75"))+
