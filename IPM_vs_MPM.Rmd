...
lifespans <- lapply(unique(tm$SPP), function(x){
  ...
  nx[1,] <- 0 # made all the reproductive and survival of reproductive zero
  jj <- 1
  while(sum(vec)>1 & !is.na(sum(vec))){
    vec=nx%*%vec
...
byspecies_sim <- lapply(unique(tm$SPP)[[5]], function(sp){
  ...
  above_l <- l_all[l_all >= mn_l]
  below_l <- l_all[l_all < mn_l]
  ...
  mn_f <- mean(fecundity[grep(sp, names(fecundity))]) 
  above_f <- 
  below_f
})
