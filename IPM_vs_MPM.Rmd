...
lifespan <- function(nx, seedlingBin = 1, fecundityRow = c(1)){
  nclasses=dim(nx)[1]
  vec <- rep(0, nclasses)
  vec[seedlingBin] <- 100
  nx[1,]=0
...
# When clustered collection, then scale Frequency (Freq) to collect X% of the time given collections for ClusteredColl consecutive years
# TotYrs/ClusteredColl # need to collect 
# I divide the percent chance by the cluster
...
# What is different if pre- or post-reproductive census 
# Can we get this from COMPADRE (just MPM)?? It would be better to translate 
SeedHarvestSim <- function(Mx_list, TMx_list, Nx, StartPopSize, GoodBadTm, Freq = c(0,0), Int = c(0,0), TotYrs, ClusteredColl = 1, seedbank = FALSE){
  ...
  # Start with good! positive; create vectors to hold distribution of good or bad years, the freequency at which a year is harvested (CollectYN), the intensity or percentage fecundity is reduced selected from the good or bad rates (depending on which it is); and the population size which I add in the inital size but discard when making a dataframe for the output (the first of the list of outputs from the function)
  Yr <- "Good"
  GBs <- rep(NA,TotYrs)
  Freqs <- rep(NA,TotYrs)
  ...
  while(yrs <= TotYrs){
    if(Yr == "Good"){
      ...
      nx <- below_f[[sample(1:length(below_f), 1)]]
        ...
        # When clust is >= ClusteredColl then proceed with random seleciton of years to harvest by defined frequencies
      ...
      CollectYN <- rbinom(1,1,Freq_m[Yr,])
        ...
        # Multiply N_t+1 = M*N_t
        # if(sum(nx%*%vec) < dendep) 
        vec <- nx%*%vec
  ...
  SimDF <- data.frame(GB_yrs = GBs, Frequency = Freqs[-1], Intensity = Intens, PopulationSize = PopSize)
...
nx <- mean(Mx_all[grep("SIAC", unlist(Nx_names))]) # some straight to seedbank, some straight to seedlings
nclasses <- dim(nx)[1]
vec <- c(0, 100, rep(0,(nclasses-2))) # since the first class is seed bank and the second class are the seedlings
  nx[1:2,] <- 0 # made all reproduction zero
jj <- 1
while(sum(vec)>1){
  (vec <- nx%*%vec)
  jj <- jj+1
...
# Check which have either seedbank or dormant 
sp_info[sp_info$Unobserved.stages>0,]


# stochastic, not deterministic, 
# For some the first two rows are reproduction (to seedbank and to seedling, need both to be zero)
lifespans <- lapply(unique(tm$SPP), function(sp){
  print(sp)
  if(sp == "CIPI"){
  ...
  if(sp %in% c("ASSC","DIFR","ERCU","HYCU","LAVE","NEMA","PHEM","PHIN","SIAC")){ # Ones where the first stage is not seedlings
...
# Density Dependence 
Can we detect density dependence from the matrices?   
No, and we don't have to. We might want to test for evidence across species/life lengths for relationship of fecundity_t ~ recruitment_t+1 and fecundity_t ~ survival of seedlings_t+1 to test for evidence of excess seed. Looking for reasons why variable percent harvest in a good or bad year would potentally have a long-term impact.   
Vital rates, elasticities, life-cycle parameters: see Silvertown et al. 1993   
```{r}
# positive density dependence == Allee-effect, can't get pollinated if too few individuals so growth rates slow vs. negative density dependence when resource is sparce
# Follow a population from starting size, multiply each matrix, plot the "linear regression of pop. growth" - RAMAS.. what would that be?

# log(sum(vec_t+1)/sum(vec_t)) is the change over time
# sqrt(t+1 - t) the amount of time between censuses

dd <- lapply(unique(tm$POP), function(POP){
  vec <- Nx_all[grep(POP,names(Nx_all))]   # POP <- "ARFE_Charleys"
  counts <- sapply(vec, function(x) sum(x))
  yrs <- tm$YR[tm$POP == POP]
  yrs_change <- sqrt(diff(yrs))
  #lag counts is the previous year count
  counts_change <- log(counts/lag(counts))/yrs_change
  lm.df <- data.frame(deltcount = counts_change[-1], year = yrs_change)
  lm1 <- lm(deltcount ~ -1 + year, data=lm.df)
  mu <- predict(lm1, level=0.95, interval="confidence", se.fit=T)
  
  plot(counts[-length(counts)],counts_change[-1])
  abline(lm1)
})

counts;lag(counts)
log(counts/lag(counts))/yrs_change; log(exp(diff(log(counts))-1))/yrs_change


# ?????

# Ryu et al 2016.
# divide abundance by average and model survival and fecundity (t to t+1) as function of relative density in year t
dd <- lapply(unique(tm$POP), function(POP){
  vec <- Nx_all[grep(POP,names(Nx_all))]   # POP <- "ARFE_Charleys"
  counts <- sapply(vec, function(x) sum(x))
  avg_count <- mean(counts)
  relative.density <- counts/avg_count
  # how to get survival alone from MPM???
  # lambda instead?
  l_pop <- lambdas[grep(POP, names(lambdas))]
  lm.df <- data.frame(lambda = unlist(l_pop), rel.den = relative.density)
  lm.df
  })
...
# Looks like density dependence to me. 
plot(dd[[1]]$rel.den, dd[[1]]$lambda, xlab="lambda", ylab="relative density", main = unique(tm$SPP)[1])

```

## Or Langton 2002 Estimation of Density Dependence   
x_t - x_t-1 + a + bn_t + e_t   
x_t is log(abundance)    
n_t abundance at time t    
e_t random error
where Pollard et al. 1987 used stochastic Gompertz model      
x_t - x_t-1 + a + bx_t-1 + e_t   

better to write as the proportional change   
d_t = x_t - x_t-1   
d_t = b(x_t-1 - x_e) + e_t     
x_e is equilibrium size on log-scale

```{r}

# This isn't the equilibrium size...
ststage <- lapply(Mx_all, function(x) stable.stage(x))

# Regression estimator among sites per species 

# SPP <- unique(tm$SPP)[20]
dd.site <- lapply(unique(tm$SPP), function(SPP){
  # POP <- unique(tm$POP[tm$SPP==SPP])[1]
  lm.df <- do.call(rbind,lapply(unique(tm$POP[tm$SPP==SPP]), function(POP){
    vec <- Nx_all[grep(POP,names(Nx_all))]   # POP <- "ARFE_Charleys"
    counts <- sapply(vec, function(x) sum(x, na.rm = TRUE))
    dt1 <- log(counts[-1]) - log(counts[-length(counts)])
    x_t0 <- log(counts[-length(counts)])
    l_pop <- lambdas[grep(POP, names(lambdas))]
    lm.df <- data.frame(dt1, x_t0, lmbd = unlist(l_pop[-length(l_pop)]), Site = POP, Species = SPP)
    lm.df
    }))
  lm.df
  })

dd.all <- do.call(rbind,dd.site)

ggplot(dd.all, aes(x_t0, dt1, colour = Site))+
  stat_smooth(method = "lm", show.legend = FALSE, alpha = 0.5)+
  facet_wrap(~Species, scales="free")+
  theme_bw()+
  geom_point(show.legend=FALSE)+
  ylab(expression('log change in abundance'['t to t+1']))+
  xlab(expression('log abundance'[t]))
  

ggplot(dd.all, aes(x_t0, lmbd, colour = Site))+
  stat_smooth(method = "lm", show.legend = FALSE, alpha = 0.5)+
  facet_wrap(~Species, scales="free")+
  theme_bw()+
  geom_point(show.legend=FALSE)+
...
# Simulations
## Test output for one species - one starting population size, 1000 replicates, no harvest to equal harvest good vs. bad to larger percentage in good to larger in bad, equal frequency 
```{r}
...
# Discrete rates
Intensityrates <- cbind(rbind(seq(0,1,by=0.1),seq(0,1,by=0.1)),combn(seq(0,1,by=0.1),2))
FreqRates <- combn(c(0,0.1,0.),2)

# Freq <- data.frame(Good = seq(0,1,by=0.1)) # 20% chance of collecting in a good year, 5% in a bad year 
# Int <- c(0.2,0.5)   # Collect 20% of seed (i.e. reduce fecundity by 20%) if a good year colleciton and by 50% if a bad year collection
totalyears <- 100
...
compare_DIFR <- do.call(rbind, lapply(1:100, function(x){
  DIFR_rep <- SeedHarvestSim(Mx_list = Mx_all[grep("DIFR", unlist(Nx_names))], 
                          TMx_list = TMx_all[grep("DIFR", unlist(Nx_names))],
                          Nx = stable.stage(mean(Mx_all[grep("DIFR", unlist(Nx_names))])),
                          StartPopSize = 500,
                          GoodBadTm = GBmatrix,
  ...
  DIFR_rep[[1]]$Replicate <- x
  DIFR_rep[[1]]
}))


compare_DIFR$Year <- 1:totalyears
ggplot(compare_DIFR, aes(Year, PopulationSize, colour = as.factor(Replicate)))+
  geom_line()+
  theme(legend.position="none")+
  geom_rug(colour = as.numeric(compare_DIFR$GB_yrs), sides="b")
...
##### Make S4 objects later as output of simulation - optional
```{r}
...
plot(testfoo[[1]]$PopulationSize, col=testfoo[[1]]$GB_yrs)
...
sizes_tenpercent <- lapply(c(10, 50, 100, 500, 1000), function(popsz){
  out <- lapply(unique(tm$SPP)[1], function(sp){
    if(sp %in% c("ASSC","DIFR","ERCU","HYCU","LAVE","NEMA","PHEM","PHIN","SIAC")){ # first stage not seedlings
      sb <- "TRUE"
    } else {
        sb <- "FALSE"
      }
    replicates <- lapply(1:100, function(x){
            SeedHarvestSim(Mx_all[grep(sp, unlist(Nx_names))],
...
plot(sizes_tenpercent[[1]][[1]][[1]]$PopulationSize, type="l", main="start pop size 10",
     xlab = "Year", ylab = "Population size")
for(i in 2:100){
  lines(sizes_tenpercent[[1]][[1]][[i]]$PopulationSize)
}

plot(sizes_tenpercent[[2]][[1]][[1]]$PopulationSize, type="l", main="start pop size 50",
     xlab = "Year", ylab = "Population size")
for(i in 2:100){
  lines(sizes_tenpercent[[2]][[1]][[i]]$PopulationSize)
}

plot(sizes_tenpercent[[3]][[1]][[1]]$PopulationSize, type="l", main="start pop size 100",
     xlab = "Year", ylab = "Population size")
for(i in 2:100){
  lines(sizes_tenpercent[[3]][[1]][[i]]$PopulationSize)
}


plot(sizes_tenpercent[[4]][[1]][[1]]$PopulationSize, type="l", main="start pop size 500",
     xlab = "Year", ylab = "Population size")
for(i in 2:100){
  lines(sizes_tenpercent[[4]][[1]][[i]]$PopulationSize)
}

plot(sizes_tenpercent[[5]][[1]][[1]]$PopulationSize, type="l", main="start pop size 1000",
     xlab = "Year", ylab = "Population size")
for(i in 2:100){
  lines(sizes_tenpercent[[5]][[1]][[i]]$PopulationSize)
}
```
