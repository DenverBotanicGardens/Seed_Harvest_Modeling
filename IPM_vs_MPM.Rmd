...
SeedHarvestSim <- function(Mx_list, TMx_list, Nx, StartPopSize, GoodBadTm, Freq = c(0,0), Int = c(0,0), TotYrs, ClusteredColl = 1, fecundityRows = c(1), seedbank = FALSE){
  ...
  while(yrs < TotYrs){
    if(Yr == "Good"){
      ...
      CollectYN <- rbinom(1,1,Freq_m[Yr,])
        ...
        if(CollectYN == 1){  # yes collect, reduce fecuntity by the year good or bad
          if(seedbank == TRUE){ 
            nx[fecundityRows,-1] <- nx[fecundityRows,-1]*Inten_m[Yr,] 
            } else {
              nx[fecundityRows,] <- nx[fecundityRows,]*Inten_m[Yr,]
            }
          }
        ...
        yrs <- yrs+1
        print(paste("not a cluster",yrs))
        # can't start a cluster if already at the end
        if(yrs > TotYrs) break
    ...
    if(Freqs[yrs-1] == 1){
      ...
      while(clust < ClusteredColl){
          ...
          clust <- clust+1 # This is the second year of a cluster
          if(Yr == "Good"){
            nx <- above_f[[sample(1:length(above_f), 1)]] # randomly sample from the high fecundty transition matrices
          } else {
            nx <- below_f[[sample(1:length(below_f), 1)]]
...
FreqRates <- combn(c(0,0.1,0.2,0.5,0.75,1),2)
...
compare_DIFR <- do.call(rbind, apply(Intensityrates, 2, function(i){
  outf <- do.call(rbind, apply(FreqRates, 2, function(f){
    compare_DIFR <- do.call(rbind, lapply(1:100, function(x){
      ...
      DIFR_rep[[1]]$Year <- 1:100
      DIFR_rep[[1]]$GoodFreq <- f[1]
      DIFR_rep[[1]]$BadFreq <- f[2]
      DIFR_rep[[1]]$GoodInt <- i[1]
      DIFR_rep[[1]]$BadInt <- i[2]
      DIFR_rep[[1]]
      }))
    compare_DIFR
    }))
