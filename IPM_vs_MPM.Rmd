...
x <- unique(tm$SPP)[3]
# Help! can't figure out errors with CIPI and why are some crazy long-lived?? ASSC has dormant as stage 1; Starting with all individuals as dormant. The life length calculation doesn't allow for that...
# unlimited growth! Clearly there must be some density dependency
...
Freqmatrix <- matrix(c(0.75,0.25), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Freq")))
Intmatrix <- matrix(c(0.25,0.75), nrow=2, byrow = TRUE, dimnames = list(c("Good","Bad"), c("Inten")))
...
## Mx_list list of annual transition matrices
## TMx_list list of annual transition matrices without fecundity
...
plot(testfoo$PopulationSize, type="l");plot(testfoo_rand$PopulationSize, type="l")
```


multiply out and take the number of years to below a threshold (sum(vec)) of sizes from different starting sizes with different seed harvest
```{r}
